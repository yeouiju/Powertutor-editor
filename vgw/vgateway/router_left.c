...
void make_ether_frame_arp_req(uint8_t* buf) {
    ...
    memcpy(arp->arp_spa, "\x0a\x01\x01\x01", 4);
    memcpy(arp->arp_tha, dest_tap_addr, ETH_ALEN);
    memcpy(arp->arp_tpa, "\x0a\x01\x01\x02", 4);
}
...
int main() {
    ...
    struct in_addr addr;
    while (1) {
        printf("Trying to send ARP request... %ld bytes\n", sizeof(buf));
