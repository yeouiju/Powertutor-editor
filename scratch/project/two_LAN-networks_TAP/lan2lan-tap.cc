...
main (int argc, char *argv[])
{
  ...
  LogComponentEnable ("TapBridgeHelper", ns3::LOG_LEVEL_LOGIC);
  ...
  NetDeviceContainer p2p_devs = p2p_link1.Install (router1, router2);
  ...
  Ipv4InterfaceContainer link1_interface = address.Assign (p2p_devs);
  ...
  tapBridge.Install (router1, p2p_devs.Get (0));

  for (uint32_t i = 0; i < lan1_nodes.GetN (); i++)
    {
      std::cout << "Echo Client " << i << " " << lan1_interfaces.GetAddress (i) << "\tserver "
  ...
  csma1.EnablePcapAll ("TAP-Lan1");
  csma2.EnablePcapAll ("TAP-Lan2");
  p2p_link1.EnablePcapAll ("TAP-Router");
